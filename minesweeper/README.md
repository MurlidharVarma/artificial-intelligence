...
## Screencast
[![Project 1b: Minesweeper](https://img.youtube.com/vi/kC_FMX4h47E/0.jpg)](https://youtu.be/kC_FMX4h47E)
...
Function `add_knowledge` accept a cell (represented as a tuple (i, j)) and its corresponding count, and update self.mines, self.safes, self.moves_made, and self.knowledge with any new information that the AI can infer, given that cell is known to be a safe cell with count mines neighboring it.

Function `make_safe_move` returns a move (i, j) that is known to be safe.

Function `make_random_move` returns a random move (i, j).